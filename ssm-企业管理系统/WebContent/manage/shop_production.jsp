	...
	<div class="admin-content-body">
		<div class="am-cf am-padding am-padding-bottom-0">
			<div class="am-fl am-cf">
				<strong class="am-text-primary am-text-lg">生产记录管理</strong><small></small>
			</div>
		</div>
					...
					<table class="am-table am-table-striped am-table-hover table-main">
						<thead>
							<tr>
								...
								<th class="table-number">生产数量</th>
								<th class="table-date">生产日期</th>
							...
							<ul class="am-pagination">
								<li class="">
									<c:if test="${pageInfo.hasPreviousPage }">
										<a href="../shop_production?pageNo=${pageInfo.pageNum-1}">«</a>
									</c:if>
								</li>
								<c:forEach begin="1" end="${pageInfo.pages}" var="pageNum">
									<li>
										<span class="color"><a href="../shop_production?pageNo=${pageNum}">${pageNum}</a></span>
									</li>
								</c:forEach>
								<li>
	...
	<script type="text/javascript">
		...
		function addshop(title, id) {
			$.jq_Panel({
				...
				iframeWidth : 500,
				iframeHeight : 300,
				url : "shop_add_production.jsp"
			});
		}
		function deleteshop(title, id,s_id,number,date) {
			 if (window.confirm ("你确定真的要删除吗")){
				 window.location.href="../deleteshopproduct?s_id="+s_id+"&number="+number+"&date="+date+"&id="+id+"&page="+${pageInfo.pageNum};
				 alert(id+"已经被删除");
				 }else{
					 alert("你已取消删除"); 
				 }
		...
		function updateshop(title,id,s_id,name,number,date) {
			...
			$.jq_Panel({
				...
				iframeWidth : 500,
				iframeHeight : 300,
				url : "shop_update_production.jsp?id="+id+"&s_id="+s_id+"&s_name="+name+"&number="+number+"&date="+date+"&page="+page
			});
		    }
		function searchshop() {
			var name = document.getElementById("shop_search").value
			window.location.href="../searchshopproduct?name="+name;
		}
	</script>
